...
mod rust_server {
    ...
    use std::thread;
    use dataflow::dataflow_service_server::DataflowServiceServer;
    ...
    use dataflow::dataflow_service_server::DataflowService;
    use dataflow::{*};
    ...
    fn start_server() -> PyResult<()> {
        thread::spawn(|| {
            ...
            rt.block_on(async {
                ...
                let addr = format!("{}:50051", bind_ip).parse().unwrap();
                let dataflow = MyDataflowService::default(); // Make sure this is implemented properly
